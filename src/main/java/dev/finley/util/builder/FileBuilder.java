...
public class FileBuilder {
    ...
    public static FileBuilder getFileBuilder(String path, String file) {
        if (!fileBuilderCache.containsKey(file)) return new FileBuilder(path, file);
    ...
    public String getString(String path) {
        if (!contains(path)) {
            Printer.sendMessage("en", "error.language.notFound");
            return "&cInvalid data... :(";
        }
        return this.yaml.getString(path);
    }
    ...
    public int getInteger(String path) {
        if (!contains(path)) return -1;
        return this.yaml.getInt(path);
    }

    public long getLong(String path) {
        if (!contains(path)) return -1L;
        return this.yaml.getLong(path);
    }

    public double getDouble(String path) {
        if (!contains(path)) return -1.0D;
